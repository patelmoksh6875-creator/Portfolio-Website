...
- The carousel is a real carousel, not a coverflow row: cards don't just shrink/rotate in place, they get pulled horizontally back toward the center X position as they lose focus (`pulledX` in `updateCarouselTransforms()`, `js/script.js`) — so a card visibly slides behind the front one and gets mostly hidden there, rather than spreading out toward the screen edges. The pull is eased (`Math.pow(pullT, 1.8)`) so the first card out still arcs outward and stays clearly visible, then the pull ramps up fast and it's mostly tucked behind by ~2.5 cards away — "goes out, then gets hidden," not an immediate snap. Combined with `rotateY`, `translateZ` (pushed back), shrinking `scale`, and fading `opacity`, all driven by live distance from center and recalculated every scroll frame via `requestAnimationFrame`. `.gate-carousel{perspective:1500px}` is what makes the rotation read as 3D. Only the centered card shows its title/artist text (everything else fades that text out) to keep it uncluttered.
- Scrolling is native (`scroll-snap-align:center`) — trackpad, touch, and the arrow buttons (`#gate-prev`/`#gate-next`, via `scrollIntoView`) all drive the same transform recalculation, no custom drag logic needed.
...
- Placeholder audio paths are `assets/audio/track-1.mp3` through `track-4.mp3` — see `assets/audio/README.md` for exact filenames to drop in. Each card's `.gate-art` is a plain CSS gradient placeholder (`style="background:linear-gradient(...)"` inline on the element) — swap each for a real `<img>` once artwork is ready. Note: `updateGateBackground()` currently copies the inline gradient value straight across; once real artwork images are added, that function needs a small tweak (e.g. reading the image and setting it as a blurred `background-image` instead) to keep the dynamic-background effect working.
