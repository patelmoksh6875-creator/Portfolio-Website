...
- The carousel is a real coverflow: every card gets a continuous `rotateY` + `translateZ` + `translateY` + `scale` + `opacity` transform driven by its live distance from the carousel's visual center (`updateCarouselTransforms()`, recalculated on every scroll frame via `requestAnimationFrame`) — so as you scroll past a card it visibly turns and tucks back behind the next one in an arc, rather than just shrinking in place. `.gate-carousel{perspective:1200px}` is what makes the rotation read as 3D. Only the centered card shows its title/artist text (everything else fades that text out) to keep the arc uncluttered.
