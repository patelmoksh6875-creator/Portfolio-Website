...
**Category 1 — DJing / Music Production** (`.about-category[data-category="djing"]`): banner = `dj-mixer.jpg`, spin = `vinyl-record.jpg` (the CD), float = `soundwave.jpg`. Gallery is the user's own DJ mixes — **not** the site's background-music tracks, which are intentionally excluded from this section (those live only in the pre-screen gate and mini player).

**Category 2 — Cinematography** (`.about-category[data-category="cinematography"]`): banner = `comiccamera.jpg` (illustrated hand-with-camera), spin = `cameralens.jpg` (a lens front element — naturally circular, reads well turning in 3D), float = `cameraroll.jpg` (a spiraling film strip). Gallery is 6 placeholder "reel" cards waiting on real short films/clips.
