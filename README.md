...
- `.about-vinyl` (`#about-vinyl`) — `assets/about/vinyl-record.jpg` in a circular frame that **spins in proportion to scroll position**, not on a timer: `updateAboutVinylSpin()` in `js/script.js` computes 0–1 progress from the element's position in the viewport and sets `transform:rotate(progress * 720deg)` directly — no CSS transition on that property, since it must track scroll 1:1. Only runs while `#page-about` is the active page (checked every scroll-frame) and is also re-synced on navigating into About via `showPage()`.
- `.about-soundwave` — `assets/about/soundwave.jpg` as a low-opacity full-width divider strip between the intro and the track grid.
- `.about-track-grid` (`#about-track-grid`) — glass-card track browser (`backdrop-filter: blur`, matching the music gate's visual language), **built dynamically from the existing `gateCards` array** rather than a duplicate track list — same 8 tracks, same `<audio id="bg-audio">` element, same `playTrackAtIndex()`. Clicking a card plays that track and reveals/updates the mini player, so "now playing" always stays in sync between the gate, the mini player, and this page. Re-renders on the audio element's native `play`/`pause` events so the "currently playing" highlight stays correct no matter where playback was started/stopped.
