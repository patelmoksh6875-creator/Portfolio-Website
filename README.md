...
**Category 1 — DJing / Music Production** (`.about-category[data-category="djing"]`):
- `.about-banner` — a full-bleed intro panel using `assets/about/dj-mixer.jpg` as a CSS `background-image` (not `<img>`, so it degrades gracefully to a plain dark background if the file is missing, instead of a broken-image icon) with a dark gradient overlay and bold "DJING / music production" text. Not click-gated — it's just the first full-height panel; scrolling down reveals the rest normally (no scroll-hijacking).
- `.about-cd` (`#about-cd`) — `assets/about/vinyl-record.jpg`, isolated from its black background with `mix-blend-mode:screen` (the photo's black pixels become invisible against the page's own black `#page-about` background, so only the bright disc shows — no visible photo rectangle, no manual image editing needed). It **turns in real 3D as the user scrolls**: `.about-cd-wrap{perspective:1000px}` plus `transform:rotateY(progress * 720deg)` set by `updateAboutVinylSpin()` in `js/script.js`, which computes 0–1 scroll progress from the element's viewport position — no CSS transition on that property, since it must track scroll 1:1. Only runs while `#page-about` is active (checked every scroll-frame) and re-synced on navigating into About via `showPage()`.
- `.about-soundwave-dock` — `assets/about/soundwave.jpg`, same blend-mode isolation trick, docked as a small (180px) strip to the right edge of the section rather than a full-width banner, vertically centered and continuously bobbing up/down via a CSS `@keyframes` float (not scroll-linked — ambient motion, "moves like a wave"). On narrow screens it drops out of absolute positioning into normal flow, centered below the CD.
- `.about-mix-grid` — a gallery of the user's own DJ mixes (**not** the site's background-music tracks — those are intentionally excluded from this section). Currently 6 placeholder `.about-mix-card` entries (dashed-border glass cover, title, genre/year tag, disabled play button) waiting on real mix audio + cover photos. Once those exist, replace the placeholder markup per card in `index.html` — add a real `<img>` in `.about-mix-cover` and wire the play button up to actual audio (a natural next step: reuse the same `<audio id="bg-audio">` + mini-player pattern the site already has, once there's real mix files to point at).
- `playSfx(name)` — a placeholder hook (`sfxFiles` lookup, safe no-op if a name has no file registered) wired to hover/click on the track cards. Add real files under `assets/about/` and register them in `sfxFiles` in `js/script.js` to activate — no other code changes needed.
- `assets/about/README.md` documents the exact filenames expected (`dj-mixer.jpg`, `vinyl-record.jpg`, `soundwave.jpg`).

More categories (photography, design, etc.) can be added later as sibling `.about-category` blocks — the banner + category pattern is self-contained per category, nothing here assumes DJing is the only one.
