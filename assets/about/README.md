...
- **DJing's mixes**: the first two cards are real — `meri zindagi hai tu x raindance` (`mix-01.mp3` / `mix-01-flstudio.mp4`) and `pour it up x don't like` (`mix-02.mp3` / `mix-02-flstudio.mp4`). The other 4 still show the default CSS-drawn vinyl graphic (`.mix-vinyl-icon` — concentric grooves + a center label, no image) with empty `data-mix-src`/`data-fl-src`. To fill in another: drop its audio file and FL Studio screen-recording here, then set that card's `data-mix-src="assets/about/mix-0N.mp3"` and `data-fl-src="assets/about/mix-0N-flstudio.mp4"` (plus real `data-mix-title`/`data-mix-meta`, and the matching `.about-mix-title`/`.about-mix-meta` text) in `index.html` — no other changes needed, `js/script.js` picks it up automatically. The two FL Studio clips were originally 354MB/213MB screen recordings; re-encoded to 1512×828 @ 30fps H.264 (no audio track — silent to begin with) at ~8.7MB/~9.8MB.
